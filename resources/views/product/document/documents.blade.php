...
@section('action-buttons')
    <a href="{{ url('product/documents/new') }}" class="btn btn-sm header-btn">
        <i class="fa fa-cart-plus"></i> <span>Agregar documento</span>
    </a>
    <div class="btn-group">
        ...
        <div class="dropdown-menu">
            ...
            <a class="dropdown-item" href="{{ url('product/documents') . '?' . $params }}">10 documentos</a>
            <?php $params = []; $params['pages'] = 20; $params = http_build_query(array_merge($params, Request::except('pages'))); ?>
            <a class="dropdown-item" href="{{ url('product/documents') . '?' . $params }}">20 documentos</a>
            <?php $params = []; $params['pages'] = 30; $params = http_build_query(array_merge($params, Request::except('pages'))); ?>
            <a class="dropdown-item" href="{{ url('product/documents') . '?' . $params }}">30 documentos</a>
        </div>
    </div>
@endsection
                                    ...
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        @foreach($documents as $document)
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-sm btn-secondary" href="{{ url('product/documents/edit', ['id' => $document->id]) }}"><i class="fa fa-edit"></i></a>
                                                    <a class="btn btn-sm btn-secondary" target="_blank" href="{{ url('product/documents/pdf', ['id' => $document->id]) }}"><i class="fa fa-file-pdf-o"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
