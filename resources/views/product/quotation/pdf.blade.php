...
<div id="page-wrap">
    <table width="100%" style="padding: 0; margin: 0;">
        ...
        <tr>
            <td style="text-align: center; padding: 20px 0px;">
                <strong>Recibo:</strong> {{ $quotation->id }}<br>
                <strong>Fecha:</strong> {{ $quotation->created_at->format('d/m/Y H:i A') }}<br>
            </td>
        </tr>
        <tr>
            <td >&nbsp;</td>
        ...
        <tr>
            <td>
                <div>
                    <strong>Nombre:</strong> {{ $quotation->client->full_name }} <br>
                    <strong>NIT:</strong> {{ $quotation->nit }} <br><br>
                    <table width="100%">
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ number_format($quotation->surcharge, 2) }}</td>
                        </tr>
                        <tr>
                            <td width="40%"><strong>Total</strong></td>
                            <td>{{ number_format((($quotation->total - $quotation->discount) + $quotation->surcharge), 2, '.', ',') }}</td>
                        </tr>
                    </table>
                </div>
            </td>
    ...
    <table width="100%">
        ...
        @foreach ($quotation->paymentMethods as $paymentMethod)
            <tr >
    ...
    <table width="100%" class="outline-table">
        ...
        @foreach ($quotation->quotationDetails as $quotationDetail)
            <tr>
                <td class="border-right left">{{ $quotationDetail->quantity }}</td>
                <td class="border-right left">{{ $quotationDetail->product->description }}</td>
                <td class="border-right right">{{ number_format($quotationDetail->unit_price, 2, '.', ',') }}</td>
                <td class="right">{{ number_format($quotationDetail->sub_total, 2, '.', ',') }}</td>
            </tr>
        @endforeach
        ...
        <tr class="border-right">
            ...
            <td class="right border-top">{{ $quotation->total }}</td>
        </tr>
        </tbody>
    </table>
    <p>&nbsp;</p>
    ...
    <table>
        ...
        <tr>
            <td>
                <strong>Descripción:</strong> <br> {{ $quotation->description }}
            </td>
        </tr>
