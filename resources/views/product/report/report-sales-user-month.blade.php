                ...
                <div class="col-md-8 offset-md-2">
                    <div class="card card-block">
                        <div class="title-block">
                            <h3 class="title">Reporte Ventas Usuario por Mes</h3>
                        </div>
                        <form id="report-cash-day-form">
                            {{ csrf_field() }}
                            <div class="form-group{{ $errors->has('client') ? ' has-error' : '' }}">
                                <label for="client" class="control-label">Mes</label>
                                {{ Form::select('date', $months, null, ['class' => 'form-control underlined chosen-select', 'data-placeholder' => 'Seleccione un mes', 'id' => 'cash-day-date']) }}
    ...
    <script>
        $(function () {
            ...
            $reportCashDayForm.on('submit', function (e) {
                ...
                $.ajax({
                    url: '{{ url('product/reports/sales/user/month') }}',
                    type: 'POST',
                    data: $this.serialize(),
                    success: function (res) {
                        $this[0].reset();
                        $('#cash-day-date').val('').trigger('chosen:updated');
